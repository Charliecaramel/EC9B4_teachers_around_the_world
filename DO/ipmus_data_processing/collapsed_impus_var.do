...
* 1. Occupation shares based on three broad occ classifications
	...
	bys year: egen teacher_share = wtmean(teachers) if employed == 1 & occ_type!= . , weight(perwt)
	
	gen hs = 0
	replace hs = 1 if occ_type == 2
	replace hs = . if employed == 0
	replace hs = . if occ == . 
	bys year: egen hs_share = wtmean(hs) if employed == 1 & occ_type!= ., weight(perwt)
	
	gen ls = 0 
	replace ls = 1 if occ_type == 1
	replace ls = . if employed == 0
	replace ls = . if occ == . 
	bys year: egen ls_share = wtmean(ls) if employed == 1 & occ_type!= ., weight(perwt)
	
	gen hs_type = 0
	replace hs_type = 1 if hs == 1
	replace hs_type = 2 if professionals == 1
	replace hs_type = . if occ == . 
	
	gen hs_prof = 0
	replace hs_prof = 1 if hs_type == 2 & teacher_type == .
	replace hs_prof = . if hs_type == .
	bys year: egen hs_prof_share = wtmean(hs_prof) if employed == 1 & occ_type!= ., weight(perwt)
	
	gen hs_other = 0
	replace hs_other = 1 if hs_type == 1 & teacher_type == .
	replace hs_other = . if hs_type == .
	bys year: egen hs_other_share = wtmean(hs_other) if employed == 1 & occ_type!= ., weight(perwt)

* 2. Shares of teachers in based on teacher type
	gen teachers_pri = 0
	replace teachers_pri = 1 if teacher_type == 1
	replace teachers_pri = . if employed == 0
	bys year: egen teacher_share_pri = wtmean(teachers_pri) if employed == 1 & occ_type!= ., weight(perwt)
	
	gen teachers_sec = 0
	replace teachers_sec = 1 if teacher_type == 2
	replace teachers_sec = . if employed == 0
	bys year: egen teacher_share_sec = wtmean(teachers_sec) if employed == 1 & occ_type!= . , weight(perwt)
	
	gen teachers_he = 0
	replace teachers_he = 1 if teacher_type == 3
	replace teachers_he = . if employed == 0
	bys year: egen teacher_share_he = wtmean(teachers_he) if employed == 1 & occ_type!= ., weight(perwt)
	
	gen teachers_prisec = 0
	replace teachers_prisec = 1 if teacher_type == 5
	replace teachers_prisec = . if employed == 0
	bys year: egen teacher_share_prisec = wtmean(teachers_prisec) if employed == 1 & occ_type!= ., weight(perwt)
	
	gen teachers_secter = 0
	replace teachers_secter = 1 if teacher_type == 6
	replace teachers_secter = . if employed == 0
	bys year: egen teacher_share_secter = wtmean(teachers_secter) if employed == 1 & occ_type!= ., weight(perwt)
	
	gen teachers_unspec = 0
	replace teachers_unspec = 1 if teacher_type == 6
	replace teachers_unspec = . if employed == 0
	bys year: egen teacher_share_unspec = wtmean(teachers_unspec) if employed == 1 & occ_type!= ., weight(perwt)
...
* 4. Share of teachers amongst indivdiuals holding university degrees
	...
	bys year: egen teacher_share_uni = wtmean(teachers_uni) if (employed == 1 & university == 1 & occ_type!= .), weight(perwt)
	
	gen hs_uni = hs * university
	bys year: egen hs_share_uni = wtmean(hs_uni) if (employed == 1 & university == 1 & occ_type!= .), weight(perwt)

	gen ls_uni = ls * university
	bys year: egen ls_share_uni = wtmean(ls_uni) if (employed == 1 & university == 1 & occ_type!= .), weight(perwt)
	
	bys year: egen teacher_share_uni_f = wtmean(teachers_uni) if (employed == 1 & university == 1 & sex == 2 & occ_type!= .), weight(perwt)
	bys year: egen hs_share_uni_f = wtmean(hs_uni) if (employed == 1 & university == 1 & sex == 2 & occ_type!= .), weight(perwt)
	bys year: egen ls_share_uni_f = wtmean(ls_uni) if (employed == 1 & university == 1 & sex == 2 & occ_type!= .), weight(perwt)
	
	bys year: egen teacher_share_uni_m = wtmean(teachers_uni) if (employed == 1 & university == 1 & sex == 1 & occ_type!= .), weight(perwt)
	bys year: egen hs_share_uni_m = wtmean(hs_uni) if (employed == 1 & university == 1 & sex == 1 & occ_type!= .), weight(perwt)
	bys year: egen ls_share_uni_m = wtmean(ls_uni) if (employed == 1 & university == 1 & sex == 1 & occ_type!= .), weight(perwt)
...
* 5. Occupation shares based on three broad occ classifications by gender
	...
	* Gender shares within occupation
	bys year: egen male_share_teachers = wtmean(male) if employed == 1 & teachers == 1 & occ_type!= ., weight(perwt)
	bys year: egen male_share_hs = wtmean(male) if employed == 1 & hs == 1 & occ_type!= ., weight(perwt)
	bys year: egen male_share_ls = wtmean(male) if employed == 1 & ls == 1 & occ_type!= ., weight(perwt)
	
	bys year: egen female_share_teachers = wtmean(female) if employed == 1 & teachers == 1 & occ_type!= ., weight(perwt)
	bys year: egen female_share_hs = wtmean(female) if employed == 1 & hs == 1 & occ_type!= ., weight(perwt)
	bys year: egen female_share_ls = wtmean(female) if employed == 1 & ls == 1 & occ_type!= ., weight(perwt)
	
	* Share of occupation in the employed gender population 
	bys year: egen hs_share_f = wtmean(hs) if (employed == 1 & sex == 2 & occ_type!= .), weight(perwt)
	
	bys year: egen hs_share_m = wtmean(hs) if (employed == 1 & sex == 1 & occ_type!= .), weight(perwt)
		
	bys year: egen ls_share_f = wtmean(ls) if (employed == 1 & sex == 2 & occ_type!= .), weight(perwt)
	
	bys year: egen ls_share_m = wtmean(ls) if (employed == 1 & sex == 1 & occ_type!= .), weight(perwt)
		
	bys year: egen teachers_male_share = wtmean(teachers) if (employed == 1 & sex == 1 & occ_type!= .), weight(perwt)
	
	bys year: egen teachers_female_share = wtmean(teachers) if (employed == 1 & sex == 2 & occ_type!= .), weight(perwt)
	...
	*Share of male and female university/HS teachers amongst the university and HS employed population
	...
	bys year: egen maleteacher_share_uni = wtmean(teachers_uni_male) if (employed == 1 & sex == 1 & university == 1 & occ_type!= .), weight(perwt)
	
	gen teachers_uni_female = teachers_uni * female
	bys year: egen femaleteacher_share_uni = wtmean(teachers_uni_female) if (employed == 1 & sex == 2 & university == 1 & occ_type!= .), weight(perwt)

	gen teachers_hs_male = teachers_hs * male
	bys year: egen maleteacher_share_hs = wtmean(teachers_hs_male) if (employed == 1 & high_skill == 1 & sex == 1 & occ_type!= .), weight(perwt)
	
	gen teachers_hs_female = teachers_hs * female
	bys year: egen femaleteacher_share_hs = wtmean(teachers_hs_female) if (employed == 1 & high_skill == 1 & sex == 2 & occ_type!= .), weight(perwt)
